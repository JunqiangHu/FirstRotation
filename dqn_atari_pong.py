# -*- coding: utf-8 -*-
"""
Created on Thu Oct 13 00:39:15 2022
...
class DQN(nn.Module):
    def __init__(self, in_channels=4, num_actions=5):
        """
            in_channels: number of channel of input, choose stacked frames as different channels
...
class DQNAgent: 
    def __init__(self, in_channels = 1, action_space = [], USE_CUDA = False, memory_size = 10000, epsilon  = 1, lr = 1e-4):
        ...
        self.optimizer = optim.RMSprop(self.DQN.parameters(),lr=lr, alpha=0.95, eps = 0.01)
...
# e-greedy decay
epsilon_by_frame = lambda frame_idx: epsilon_max - (epsilon_max - epsilon_min) * (frame_idx - 1) / (final_exploration - 1)

frame = env.reset()

for i in range(frames):
    if i > final_exploration:
        epsilon = epsilon_min
    else:
